...
if [ -z "$SELECTED_WALLPAPER" ]; then
    notify-send --replace-id=899 "Wallpaper Selector" "No wallpapers found in the directory."
    exit 0
fi
...
notify-send --replace-id=899 -i "$SELECTED_WALLPAPER" "Wallpaper Changed" "$(basename "$SELECTED_WALLPAPER")."
